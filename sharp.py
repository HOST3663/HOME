...
TOKEN = '7500665941:AAGQTozBzAe2kudC2Rp1rOjRxUsfhCAfsYw'
ADMIN_USER_ID = 7446142108
...
def update_proxy_command(message):
    ...
    try:
        update_proxy()
        bot.send_message(chat_id, f"🔄 Proxy locked in. We’re untouchable. Bot by @Animelover24")
...
def notify_attack_finished(target_ip, target_port, duration):
    bot.send_message(
        ...
        f"💥 *Operation Complete. No Evidence Left Behind. Courtesy of @Animelover24*",
        parse_mode='Markdown'
    )
...
def approve_or_disapprove_user(message):
    ...
    if user_id != ADMIN_USER_ID:
        bot.send_message(chat_id, f"🚫 *Access Denied. Only @Animelover24 controls this realm.*", parse_mode='Markdown')
        return

    if len(cmd_parts) < 2:
        bot.send_message(chat_id, f"📝 *Format: /approve <user_id> <plan> <days> or /disapprove <user_id>. Reserved by @Animelover24*", parse_mode='Markdown')
        return
    ...
    if action == '/approve':
        ...
        if limit_reached:
            bot.send_message(chat_id, f"⚠️ *Plan limit reached. Access denied. Controlled by @Animelover24*", parse_mode='Markdown')
            return
        ...
        msg_text = f"*User {target_user_id} granted access – Plan {plan} for {days} days. Approved by @Animelover24*"
    else:
        users_collection.update_one(
            {"user_id": target_user_id},
        ...
        msg_text = f"*User {target_user_id} removed. Clearance by @Animelover24*"
...
def attack_command(message):
    ...
    if attack_in_progress:
        bot.send_message(chat_id, f"⚠️ *An attack is already in progress. Please wait until it completes, @Animelover24.*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, f"🚫 Unauthorized. Access limited to @Animelover24.")
            return

        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, f"🟠 Plan 🧡 is full. Reach out to @Animelover24 for upgrades.")
            return

        if user_data['plan'] == 2 and users_collection.count_documents({"plan": 2}) > 499:
            bot.send_message(chat_id, f"💥 Instant++ Plan at capacity. Contact @Animelover24.")
            return

        bot.send_message(chat_id, f"📝 Provide target details – IP, Port, Duration (seconds). Controlled by @Animelover24")
...
def process_attack_command(message):
    try:
        ...
        if len(args) != 3:
            bot.send_message(message.chat.id, f"⚠️ *Format incorrect. Use: /Attack <IP> <Port> <Duration>. Maintained by @Animelover24*", parse_mode='Markdown')
            return

        target_ip, target_port, duration = args[0], int(args[1]), args[2]

        if target_port in blocked_ports:
            bot.send_message(message.chat.id, f"🚫 *Port {target_port} restricted. Select a different entry point. Governed by @Animelover24*", parse_mode='Markdown')
            return
        ...
        bot.send_message(
            ...
            f"🔥 *Unleashing force. No turning back. Powered by @Animelover24* ⚡",
            parse_mode='Markdown'
        )
    except Exception as e:
...
def send_welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        f"👊 *Welcome to Command, Agent. Choose your directive.* Managed by @Animelover24",
        reply_markup=markup,
        parse_mode='Markdown'
    )
...
def handle_message(message):
    if message.text == "💀 Initiate Attack 🔥":
        bot.reply_to(message, f"*Command received. Preparing deployment. Stand by, @Animelover24*", parse_mode='Markdown')
        ...
        if user_data:
            ...
            response = (f"*Agent ID: {username}\n"
                        ...
                        f"Timestamp: {datetime.now().isoformat()}. Verified by @Animelover24*")
        else:
            response = f"*Profile unknown. Contact @Animelover24 for authorization.*"
        bot.reply_to(message, response, parse_mode='Markdown')
    elif message.text == "📜 Mission Brief":
        bot.reply_to(message, f"*For support, type /help or contact @Animelover24 at HQ.*", parse_mode='Markdown')
    elif message.text == "📞 Contact HQ":
        bot.reply_to(message, f"*Direct Line to HQ: @Animelover24*", parse_mode='Markdown')
    else:
        bot.reply_to(message, f"❗*Unknown command. Focus, Agent. Managed by @Animelover24*", parse_mode='Markdown')
